...
void object_detection::FeatureMatchingDetector::detect()
{
  ...
  std::map<std::string, Model3D::Ptr>::const_iterator iter;
  for (iter = models_.begin(); iter != models_.end(); ++iter)
  {

    const Model3D::Ptr& model = iter->second;
    ...
    // 1. query, 2. train
    int k = 2;
    descriptor_matcher->knnMatch(features_.descriptors, model->getDescriptors(), matches, k);
    for (size_t m = 0; m < matches.size(); m++ )
    {
      if (matches[m].size() == 2)
      {
        ...
        if (dist1 / dist2 < DISTANCE_RATIO_THRESHOLD)
        {
          ...
          world_points.push_back(model->getWorldPointForDescriptor(trainIndex));
        }
      }
    }
    ...
    if (world_points.size() > 3)
    {
      ...
      int min_inliers = model->getNumWorldPoints() / 2; // stop iteration if more inliers than this are found
